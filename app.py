...
def main():
    ...
    with tab1:
        ...
        st.markdown("---")
        # Alinhamento horizontal pedido: Gap e Distância na linha 1
        r1_c1, r1_c2 = st.columns(2)
        r1_c1.metric("GAP (mm)", f"{info['gap']}")
        r1_c2.metric("Distância Trabalho (mm)", f"{info['dist']}")
        
        # Alinhamento horizontal pedido: A, L e P na linha 2 (abaixo)
        r2_c1, r2_c2, r2_c3 = st.columns(3)
        r2_c1.metric("Altura [A]", f"{alt} mm")
        r2_c2.metric("Largura [L]", f"{larg} mm")
        r2_c3.metric("Profundidade [P]", f"{prof} mm")
    ...
    with tab2:
        ...
        if st.button("Calcular Resultados"):
            ...
            # Retorno em COLUNA (Vertical)
            st.divider()
            st.write("### Resultados:")
            st.metric("Iarc Final", f"{ia_f:.4f} kA")
            st.metric("Iarc Reduzida", f"{ia_min:.4f} kA")
            st.metric("Energia Incidente (cal/cm²)", f"{e_cal:.4f}")
            st.metric("Energia Incidente (J/cm²)", f"{e_j:.4f}")
            st.metric("Fronteira (mm)", f"{dla_f:.0f}")
    ...
    with tab3:
        if 'res' in st.session_state:
            ...
            def export_pdf():
                ...
                fields = [f"Equipamento: {r['Equip']}", f"Dimensões: {r['Dim']} mm", f"Gap: {r['Gap']} mm", f"Distância: {r['Dist']} mm", f"Iarc Final: {r['Ia']:.4f} kA", f"Iarc Reduzida: {r['IaMin']:.4f} kA", f"Energia: {r['E_cal']:.4f} cal/cm² ({r['E_j']:.4f} J/cm²)", f"DLA: {r['DLA']:.0f} mm", f"Vestimenta: {r['Cat']}"]
