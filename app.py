...
def main():
    ...
    with tab1:
        ...
        # LINHA 1: GAP E DISTÂNCIA (Fiel à Imagem)
        ...
        # LINHA 2: DIMENSÕES (Fiel à Imagem)
    ...
    with tab2:
        ...
        if st.button("Calcular Resultados"):
            ...
            ia_sts = [calc_ia_step(i_bf, gap_g, k_ia[v]) for v in]
            en_sts = [calc_en_step(ia, i_bf, gap_g, dist_d, tempo_t, k_en[v], cf) for ia, v in zip(ia_sts,)]
            dl_sts = [calc_dla_step(ia, i_bf, gap_g, tempo_t, k_en[v], cf) for ia, v in zip(ia_sts,)]
    ...
    with tab3:
        if 'res' in st.session_state:
            ...
            st.subheader(f"Gerar Laudo Técnico - {r['Equip']}")
            def export_pdf():
