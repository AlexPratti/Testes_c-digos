...
def main():
    ...
    with tab1:
        ...
        escolha = st.selectbox("Selecione o Equipamento:", list(dados_inv.keys()), key="equip_sel")
    ...
    with tab2:
        ...
        with col_in1:
            v_oc = st.number_input("Tensão Voc (kV)", 0.208, 15.0, 13.8, step=0.01, key="v_oc_in")
            i_bf = st.number_input("Curto-Circuito Ibf (kA)", 0.5, 106.0, 4.85, step=0.01, key="i_bf_in")
        with col_in2:
            config = st.selectbox("Eletrodos:", ["VCB", "VCBB", "HCB", "VOA", "HOA"], key="conf_in")
            # O 'value' agora lê o que foi selecionado na Aba 1
            gap_in = st.number_input("Gap G (mm)", value=float(info[0]), key="gap_in")
        with col_in3:
            d_trab = st.number_input("Distância D (mm)", value=float(info[1]), key="dist_in")
            tempo = st.number_input("Tempo T (ms)", value=488.0, key="tempo_in")

        # --- LÓGICA DE CÁLCULO (NBR 17227 / IEEE 1584-2018) ---
        ...
        cf = 1.28372 # Fator fixado para validação
        ...
        # --- EXIBIÇÃO DE RESULTADOS (ATUALIZAÇÃO AUTOMÁTICA) ---
        st.divider()
