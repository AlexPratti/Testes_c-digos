...
def main():
    ...
    with tab1:
        ...
        c3.metric("Altura (A)", f"{info[2]} mm"); c4.metric("Largura (L)", f"{info[3]} mm"); c5.metric("Profundidade (P)", f"{info[4]} mm")

    # --- ABA 2: CÁLCULOS (EQUAÇÕES 1 A 6) ---
    with tab2:
        with st.form("form_arco"):
            ...
            with col2:
                config = st.selectbox("Eletrodos:", ["VCB", "VCBB", "HCB"])
            ...
            btn_calc = st.form_submit_button("Calcular Resultados")

        if btn_calc:
            # --- COEFICIENTES VCB 14.3kV (NBR 17227:2025) ---
            ki = [0.005795, 1.015, -0.011, -1.557e-12, 4.556e-10, -4.186e-8, 8.346e-7, 5.482e-5, -0.003191, 0.9729]
            ke = [3.825917, 0.11, -0.999749, -1.557e-12, 4.556e-10, -4.186e-8, 8.346e-7, 5.482e-5, -0.003191, 0.9729, 0, -1.568, 0.99]
            cf = 1.28372 # Conforme seu Excel

            # 1. Corrente de Arco (Ia) - Cálculo Escalar (Acesso por índice ki[x])
            ...
            # 2. Energia Incidente (E) em cal/cm²
            ...
            cat = "Cat 2 (8 cal)" if e_cal <= 8 else "Cat 3 (25 cal)"
